...
export default function MobileEditProfileSheet({
  ...
  const [activeSport, setActiveSport] = useState<Sport>(profile.activeSport ?? 'pingpong');
  ...
  const onPick = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      toast({ title: t('File is too large'), description: t('Please select an image smaller than 2MB.'), variant: 'destructive' });
      return;
    }
    setImageFile(file);
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (auth.currentUser) {
        await updateProfile(auth.currentUser, { displayName: name.trim(), photoURL: photoURL ?? undefined });
      }
        ...
        <div className="mt-4 space-y-4">
          <div className="flex items-center gap-4">
            <Avatar className="h-16 w-16">
              ...
              <AvatarFallback>{(name || profile.name || '?').charAt(0)}</AvatarFallback>
            </Avatar>
            <div className="flex-1">
              <Label htmlFor="avatar">{t('Profile Picture')}</Label>
              <Input id="avatar" type="file" accept="image/png,image/jpeg" onChange={onPick} />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select value={activeSport} onValueChange={(v) => setActiveSport(v as Sport)}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {(Object.keys(sportConfig) as Sport[]).map((s) => (
                  <SelectItem key={s} value={s}>{sportConfig[s].name}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
