...
export default function BracketView({ tournament, onUpdate }: Props) {
  ...
  const persist = async (next: any) => {
    await updateDoc(doc(db, "tournament-rooms", tournament.id), { bracket: next, champion: next.champion ?? null, })
  ...
  const saveResults = async (next: any) => {
    ...
    const bronzeRound = next.rounds.find((r: any) => r.type === "knockoutBronze")
  ...
  if (!bracket)
    return <p className="text-center text-muted-foreground">Bracket not available</p>
  ...
  return (
    <div className="space-y-10">
      ...
      {bracket.stage === "completed" && <FinalTable stats={bracket.finalStats} />}
    </div>
  )
}

/* ───────── универсальный редактор раунда ───────── */
function RoundEditor({ round, bracket, persist, roundType }: any) {
  ...
  const save = (id: string) => {
    ...
    const next = [...matches]; next[idx] = upd
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{round.label} ({round.status})</CardTitle>
      </CardHeader>
      <CardContent>
        <ScrollArea>
          <Table>
            ...
            <TableBody>
              {matches.map((m: any) => {
                ...
                return (
                  <TableRow key={m.matchId} className={fin ? "" : "bg-accent/10"}>
                    <TableCell>
                      {m.player1?.name}{m.player1?.place ? ` (${m.player1.place})` : ""}
                    </TableCell>
                    <TableCell className="text-center">
                      <Input
                        ...
                        onChange={e => setMatches(ms => ms.map(x =>
                          x.matchId === m.matchId
                    ...
                    <TableCell className="text-right">
                      {m.player2?.name}{m.player2?.place ? ` (${m.player2.place})` : ""}
                    </TableCell>
                    <TableCell className="text-center font-semibold">
                      {fin
                        ? (m.winner === m.player1.userId ? m.player1.name : m.player2.name)
                        : "—"}
                    </TableCell>
                    <TableCell className="text-center">
                      {!fin && <Button size="sm" onClick={() => save(m.matchId)}>Save</Button>}
                    </TableCell>
                  </TableRow>
                )
              })}
        ...
        {round.status === "inProgress" && (
          <div className="flex justify-end mt-4">
            <Button
              onClick={finish}
              disabled={!matches.every((m: any) => m.matchStatus === "finished")}
            >
              Finish Round
            </Button>
          </div>
    ...
    <div className="mt-6">
      <ScrollArea>
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>Place</TableHead><TableHead>Player</TableHead>
              <TableHead>W</TableHead><TableHead>L</TableHead>
              <TableHead>PF</TableHead><TableHead>PA</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {rows.map(p => (
              <TableRow key={p.userId}>
                ...
                <TableCell>{p.place}</TableCell><TableCell>{p.name}</TableCell>
                <TableCell>{p.wins}</TableCell><TableCell>{p.losses}</TableCell>
                <TableCell>{p.pf}</TableCell><TableCell>{p.pa}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
        ...
        <ScrollArea>
          <Table>
            ...
            <TableBody>
              {stats.map(p => (
                <TableRow key={p.userId}>
                  <TableCell>{p.place}</TableCell><TableCell>
                    <a href={`/profile/${p.userId}`} className="text-blue-500 underline-offset-4">
                      {p.name}
                    </a>
                  </TableCell>
                  <TableCell>{p.wins}</TableCell><TableCell>{p.losses}</TableCell>
