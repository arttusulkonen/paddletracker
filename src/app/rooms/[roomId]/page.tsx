...
const parseFinnishDate = (str: string) => {
...
export default function RoomPage() {
  ...
  const [latestSeason, setLatest] = useState<any | null>(null)
  ...
  useEffect(() => {
    ...
    const unsubRoom = onSnapshot(roomRef, (snap) => {
      ...
      const last = data.seasonHistory?.slice().reverse().find((s: any) => Array.isArray(s.summary) || Array.isArray(s.members)) ?? null
      setLatest(last)
      setIsLoading(false)
    })
    ...
    const unsubMatches = onSnapshot(mQ, (s) => {
      const arr = s.docs.map((d) => ({ id: d.id, ...(d.data() as any) }) as Match)
      arr.sort((a, b) => parseFinnishDate(b.timestamp).getTime() - parseFinnishDate(a.timestamp).getTime())
      setRecent(arr)
    })
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      toast({ title: "Invited", description: `${newMember.name} added to room` })
      setInvite("")
    } catch {
  ...
  const addRow = () => setMatchesInput((r) => [...r, { score1: "", score2: "", side1: "", side2: "" }])
  ...
  const saveMatches = async () => {
    ...
    try {
      for (const row of matchesInput) {
        ...
        const [p1Snap, p2Snap] = await Promise.all([getDoc(doc(db, "users", player1Id)), getDoc(doc(db, "users", player2Id))])
  ...
  const finalRows = useMemo(() => {
    ...
    const arr = Array.isArray(latestSeason.summary) ? [...latestSeason.summary] : Array.isArray(latestSeason.members) ? [...latestSeason.members] : []
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              {latestSeason && (
                <div className="flex gap-2">
                  <Button size="sm" variant={viewMode === "regular" ? "default" : "outline"} onClick={() => setView("regular")}>
                    Regular
                  </Button>
                  <Button size="sm" variant={viewMode === "final" ? "default" : "outline"} onClick={() => setView("final")}>
                    Final
                  </Button>
                </div>
              )}
            </div>
            <CardDescription>{viewMode === "regular" ? "Current rankings" : "Final season standings"}</CardDescription>
          </CardHeader>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead onClick={() => toggleSort("rating")} className="cursor-pointer select-none">
                          Points
                        </TableHead>
                        <TableHead onClick={() => toggleSort("totalMatches")} className="cursor-pointer select-none">
                          Matches
                        </TableHead>
                        <TableHead onClick={() => toggleSort("winPct")} className="cursor-pointer select-none">
                          Win&nbsp;%
                        </TableHead>
                      </TableRow>
                    </TableHeader>
      ...
      <div>
        ...
        <ScrollArea className="h-[300px] border rounded-md p-3 bg-background">
          {[...members]
            ...
            .map((m) => {
              const total = (m.wins || 0) + (m.losses || 0)
              const winPct = total ? Math.round(((m.wins || 0) / total) * 100) : 0
              return (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium">
                        <a href={`/profile/${m.userId}`} className="hover:underline">
                          {m.name}
                        </a>
                      </p>
  ...
  function RecordBlock() {
    const list = members.map((m) => ({ userId: m.userId, name: m.name, rating: m.rating }))
    return (
      <Card className="md:col-span-2 shadow-md">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4 items-end">
            <PlayerSelect label="Player 1" value={player1Id} list={list.filter((p) => p.userId !== player2Id)} onChange={setP1} />
            <PlayerSelect label="Player 2" value={player2Id} list={list.filter((p) => p.userId !== player1Id)} onChange={setP2} />
          </div>
          ...
          {matchesInput.map((m, i) => (
            <MatchRowInput
              ...
              index={i}
              data={m}
              onChange={(row) => setMatchesInput((r) => r.map((v, idx) => (idx === i ? row : v)))}
