...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    const matchesRef = query(collection(db, "matches"), where("roomId", "==", roomId), orderBy("timestamp", "desc"))
  ...
  const regularPlayers = useMemo(() => {
    ...
    const sorted = enriched.sort((a, b) => {
      ...
      return (a as any)[sortConfig.key] > (b as any)[sortConfig.key] ? dir : -dir
    })
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              {latestSeason && (
                <div className="flex gap-2">
                  <Button size="sm" variant={viewMode === "regular" ? "default" : "outline"} onClick={() => setViewMode("regular")}>
                    Regular
                  </Button>
                  <Button size="sm" variant={viewMode === "final" ? "default" : "outline"} onClick={() => setViewMode("final")}>
                    Final
                  </Button>
                </div>
              )}
            ...
            {viewMode === "regular" && (
              <>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                  <p className="text-sm">Fair ranking shows players with matches ≥ average first</p>
                  <Button size="sm" variant="outline" onClick={() => setFiltered(f => !f)}>
